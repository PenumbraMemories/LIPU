...
def constrain_frequency(
    ...
    onsets, frames = onsets.copy(), frames.copy()
    if max_freq is not None:
        max_freq_idx = int(np.clip(np.floor(librosa.hz_to_midi(max_freq) - MIDI_OFFSET) + 1, 0, frames.shape[1]))
        onsets[:, max_freq_idx:] = 0
        frames[:, max_freq_idx:] = 0
    if min_freq is not None:
        min_freq_idx = int(np.clip(np.ceil(librosa.hz_to_midi(min_freq) - MIDI_OFFSET), 0, frames.shape[1]))
...
def model_frames_to_time(n_frames: int) -> np.ndarray:
    return np.arange(n_frames) * (FFT_HOP / AUDIO_SAMPLE_RATE)
